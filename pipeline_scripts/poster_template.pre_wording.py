...
# The photo is dimmed towards a neutral charcoal FIRST, then tinted towards
# the category colour. Doing it in that order is the whole trick: dimming
# towards the colour itself made every post look like a filter had been
# dropped on it, but dimming to neutral and then washing colour back in
# leaves the photo readable and still ties the frame to the category.
#
# TINT_STRENGTH is how much of that wash you get. The ladder, measured on a
# real slide:
#     0.03  basically none, the photo looks untinted
#     0.10  subtle, you notice it only next to an untinted one
#     0.14  clear category wash, photo still reads   <-- what we use
#     0.24  flat and murky, the photo stops reading
# Change this one number and rebuild -- it touches nothing but the photo.
...
# Where a brand mark comes from, best first.
#
# Google's favicon service was the only source, and a favicon is the wrong
# picture: it is whatever fits in a browser tab, so you get a cropped letter
# or a 16px smudge where the company has a perfectly good logo. Clearbit
# serves the actual mark, usually transparent and a few hundred pixels, and
# it is free with no key. DuckDuckGo sits in the middle. Google stays as the
# floor because it answers for almost anything.
#
# Each is tried in turn and the first one that returns an image BIG ENOUGH TO
# BE WORTH KEEPING wins -- without that size test the chain is pointless,
# because Google always answers and a 16px answer would be cached forever
# and block every better source from ever being tried again.
LOGO_SOURCES = [
    ("clearbit",   "https://logo.clearbit.com/{domain}",                    64),
    ("duckduckgo", "https://icons.duckduckgo.com/ip3/{domain}.ico",         48),
    ("google",     "https://www.google.com/s2/favicons?domain={domain}&sz=128", 0),
]


def fetch_brand_mark(domain, label=""):
    """The best mark any of the sources will give us for this domain, as a
    Pillow image. None if they all decline."""
    import requests
    from io import BytesIO
    for name, url, min_px in LOGO_SOURCES:
        try:
            r = requests.get(url.format(domain=domain), timeout=15)
            if r.status_code != 200 or len(r.content) < 200:
                continue
            im = Image.open(BytesIO(r.content)).convert("RGBA")
            if min_px and max(im.size) < min_px:
                # Answered, but with something too small to put on a card.
                continue
            if label:
                print(f"      brand logo: {label} <- {domain} ({name}, "
...
def _cached_under_same_name(business_name, slug):
    """A cached mark filed under a different spelling of this same employer."""
    want = _norm_tokens(business_name)
    if not want:
        return None
    try:
        names = os.listdir(LOGOS_DIR)
    except OSError:
        return None
    for f in names:
        ...
        for suffix in ("_brand", "_seek"):
            if stem.endswith(suffix):
                ...
                break
        if stem == slug:
            continue                       # the normal path already tried this
        if _norm_tokens(stem.replace("_", " ")) != want:
            continue
        p = os.path.join(LOGOS_DIR, f)
        if os.path.getsize(p) > 0:
            print(f"      logo: {business_name} <- cached as {f}")
            return p
    return None
...
def _logo_for(business_name, logo_url=None):
    ...
    # A mark already accepted for the SAME company under a slightly different
    # spelling. "SG Fleet" and "SG Fleet AU" are one employer, and the first
    # had a perfectly good logo cached while the second showed initials,
    # because the cache is keyed on the raw name.
    #
    # The test is deliberately strict: the two names have to reduce to the
    # EXACT same significant words, not merely overlap. Equal tokens means
    # the difference between them was only noise like "AU" or "Pty Ltd";
    # overlapping tokens is how "Apple" ends up on "Apple Valley Childcare".
    alt = _cached_under_same_name(business_name, slug)
    if alt:
        return alt
    ...
    if domain:
        ...
        try:
            im = fetch_brand_mark(domain, business_name)
            if im is not None:
                im.save(brand_cached, "PNG")
                return brand_cached
...
# ===========================================================================
# COVER STYLES -- the bottom "furniture" on slide 1 rotates between these
# ===========================================================================
# The headline, the count bubble and the handle stay the same on every cover
# (they're what people read). What changes is the bit around them, so the
# grid doesn't look like the same camera screen ten times in a row.
#
# SETTINGS: the number is how often a style comes up. 0 = never.
#   camera 1 and the rest 2 -> the camera shows up about half as often as
#   each of the others. Want it gone? Set it to 0. Want only camera? Set
#   everything else to 0. (The old camera-only version is saved as
#   poster_template.pre_covers.py.)
COVER_STYLES = {
    "camera":     1,   # phone viewfinder + shutter (the original)
    "clean":      2,   # just the photo and the type, nothing else
    "lockscreen": 2,   # a phone notification: "7 new Architecture roles"
    "search":     2,   # a search bar: "architecture internships sydney"
    "chat":       2,   # a text conversation instead of the grey bubble
    "ticket":     2,   # an ADMIT ONE ticket stub
    "sticky":     2,   # a yellow sticky note in the corner
    "polaroid":   2,   # the whole cover framed as a polaroid
    "calendar":   2,   # a tear-off calendar page with the posting day
    "checklist":  2,   # find roles [x]  apply [ ]  get the offer [ ]
}

# Which day each category posts -- for the calendar style.
COVER_DAY = {
    "Technology, Data & AI": "MONDAY",
    "Business, Commerce, Marketing & Finance": "THURSDAY",
    "Engineering": "SATURDAY",
    "Architecture": "WEDNESDAY",
    "Medicine & Health": "SUNDAY",
}


def pick_cover_style(seed, _week=None, _avoid=True):
    """Which style this carousel's cover gets.

    Worked out from the carousel's name + the week number, so:
      * rebuilding the same carousel in the same week gives the same cover
        (nothing changes under you while a post waits in the queue)
      * next week the same category gets a different style
      * never the same style two weeks running for the same carousel
    crc32 is just a quick way to turn text into a number (like a hash in C).
    """
    import datetime
    week = _week if _week is not None else datetime.date.today().isocalendar()[1]
    names = [n for n, w in COVER_STYLES.items() if w > 0] or ["camera"]
    pool = [n for n in names for _ in range(COVER_STYLES.get(n, 1) or 1)]
    pick = pool[zlib.crc32(f"{seed}|{week}".encode()) % len(pool)]
    if _avoid and len(names) > 1:
        last = pick_cover_style(seed, week - 1, _avoid=False)
        if pick == last:
            pick = names[(names.index(pick) + 1) % len(names)]
    return pick
...
def _drop_shadow(img, box, radius, blur=18, alpha=110):
    sh = _layer()
    ImageDraw.Draw(sh).rounded_rectangle(
        [box[0], box[1] + 8, box[2], box[3] + 8], radius=radius, fill=(0, 0, 0, alpha))
...
def _style_lockscreen(img, short, count, ink, **_):
    ...
    d.rounded_rectangle([x0, y0, x1, y1], radius=34, fill=(246, 246, 248, 235))
    d.rounded_rectangle([x0 + 26, y0 + 30, x0 + 96, y0 + 100], radius=18, fill=ink)
    f_b = _font("DejaVuSans-Bold.ttf", 40)
    d.text((x0 + 61 - d.textlength("b", font=f_b) / 2, y0 + 38), "b", font=f_b, fill="white")
    f_app, f_t, f_s = (_font("DejaVuSans-Bold.ttf", 24), _font("DejaVuSans-Bold.ttf", 32),
...
def _style_search(img, short, count, ink, **_):
    ...
    d = ImageDraw.Draw(img)
    d.rounded_rectangle([x0, y0, x1, y1], radius=45, fill=(255, 255, 255))
    cx, cy = x0 + 52, y0 + 42
    ...
    d.line([x1 - 60, y0 + 24, x1 - 60, y1 - 24], fill=ink, width=4)   # the cursor
    f_r = _font("DejaVuSans-Bold.ttf", 28)
    r = f"{count} result{'s' if count != 1 else ''} — swipe"
...
def _style_ticket(img, short, count, ink, day, **_):
    ...
    t = _layer()
    d = ImageDraw.Draw(t)
    d.rounded_rectangle([x0, y0, x1, y1], radius=18, fill=(250, 244, 230, 255))
    # the two notches where the stub tears off: punch them out to see-through
    for cy in (y0, y1):
        d.ellipse([cut - 20, cy - 20, cut + 20, cy + 20], fill=(0, 0, 0, 0))
    for yy in range(y0 + 26, y1 - 20, 18):                     # perforation
        d.line([cut, yy, cut, yy + 9], fill=(170, 160, 140, 255), width=3)
    ...
    for i in range(22):                                        # barcode
        ...
        d.rectangle([bx, y0 + 34, bx + wdt, y1 - 34], fill=(40, 34, 30, 255))
        bx += wdt + 3
        if bx > x1 - 34:
            break
...
def _style_sticky(img, short, count, ink, **_):
    ...
    d.rectangle([105, 0, 195, 34], fill=(255, 255, 255, 150))     # the tape
    ...
    x, y = W - note.width - 70, H - note.height - 60
    img.alpha_composite(sh.filter(ImageFilter.GaussianBlur(12)), (x + 6, y + 10))
    img.alpha_composite(note, (x, y))


def _style_polaroid(img, short, account_handle, **_):
    frame = _layer()
    d = ImageDraw.Draw(frame)
    side, bottom = 44, 200
    d.rectangle([0, 0, W, side], fill=(250, 249, 245, 255))
    d.rectangle([0, 0, side, H], fill=(250, 249, 245, 255))
    d.rectangle([W - side, 0, W, H], fill=(250, 249, 245, 255))
    d.rectangle([0, H - bottom, W, H], fill=(250, 249, 245, 255))
    f = _font("DejaVuSans-Bold.ttf", 46)
    txt = f"{short} — Sydney"
    d.text(((W - d.textlength(txt, font=f)) / 2, H - bottom + 50), txt, font=f,
           fill=(45, 45, 50))
    f2 = _font("DejaVuSans.ttf", 26)
    h = f"@{account_handle}"
    d.text(((W - d.textlength(h, font=f2)) / 2, H - bottom + 116), h, font=f2,
           fill=(130, 130, 136))
...
def _style_calendar(img, short, ink, day, **_):
    ...
    d = ImageDraw.Draw(img)
    d.rounded_rectangle([x0, y0, x1, y1], radius=22, fill=(255, 255, 255))
    d.rounded_rectangle([x0, y0, x1, y0 + 70], radius=22, fill=ink)
    d.rectangle([x0, y0 + 40, x1, y0 + 70], fill=ink)
    for rx in (x0 + 70, x1 - 70):                                # the rings
        d.ellipse([rx - 9, y0 - 12, rx + 9, y0 + 6], fill=(60, 60, 66))
...
def _style_checklist(img, short, ink, **_):
    x0, y0, x1, y1 = 200, 830, W - 200, 1030
    _drop_shadow(img, (x0, y0, x1, y1), 26)
    d = ImageDraw.Draw(img)
    d.rounded_rectangle([x0, y0, x1, y1], radius=26, fill=(255, 255, 255))
    f = _font("DejaVuSans-Bold.ttf", 32)
    items = [(f"find {short.lower()} roles", True), ("apply", False), ("get the offer", False)]
    y = y0 + 28
    for text, done in items:
        bx, by = x0 + 36, y + 4
        if done:
            d.rounded_rectangle([bx, by, bx + 40, by + 40], radius=8, fill=ink)
            d.line([bx + 9, by + 21, bx + 17, by + 30, bx + 32, by + 11], fill="white", width=6)
        else:
            d.rounded_rectangle([bx, by, bx + 40, by + 40], radius=8,
                                outline=(170, 170, 178), width=4)
        ff = f
        while d.textlength(text, font=ff) > (x1 - x0 - 130) and ff.size > 20:
            ff = _font("DejaVuSans-Bold.ttf", ff.size - 2)
        d.text((bx + 64, y + 4), text, font=ff,
               fill=(150, 150, 158) if done else (30, 30, 34))
        if done:
            tw = d.textlength(text, font=ff)
            d.line([bx + 64, y + 26, bx + 64 + tw, y + 26], fill=(150, 150, 158), width=3)
        y += 60
...
def _chat_bubbles(draw, yy, short, msg, ink):
    ...
    draw.ellipse([x0 - 12, yy + h - 26, x0 + 12, yy + h - 2], fill=(233, 233, 235))
    draw.text((x0 + 35, yy + 18), q, font=f, fill=(28, 28, 30))
    yy += h + 22
    a = f"{msg} — swipe"
    tw = draw.textlength(a, font=f)
    x1 = W - 150
    draw.rounded_rectangle([x1 - tw - 70, yy, x1, yy + h], radius=h / 2, fill=ink)
    draw.ellipse([x1 - 12, yy + h - 26, x1 + 12, yy + h - 2], fill=ink)
    draw.text((x1 - tw - 35, yy + 18), a, font=f, fill=(255, 255, 255))


def build_cover(category, count, out_dir, account_handle="[your account handle]",
                filename="slide_01_cover.jpg", seed="cover", n_grad=0, style=None):
    """
    ...
    if style == "camera":
        ...
        _style_lockscreen(img, **extras)
    elif style == "search":
        _style_search(img, **extras)
    elif style == "ticket":
        _style_ticket(img, **extras)
    elif style == "sticky":
        _style_sticky(img, **extras)
    elif style == "polaroid":
        _style_polaroid(img, **extras)
    elif style == "calendar":
        _style_calendar(img, **extras)
    elif style == "checklist":
        _style_checklist(img, **extras)
    # "clean" and "chat" add nothing down the bottom
    draw = ImageDraw.Draw(img)
